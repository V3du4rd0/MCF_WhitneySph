...
if (row<Np && col<2 ){
	...
	if (row==0 ){
		...
		r2=pow(M[row*2],2)+pow(M[row*2+1],2);
		zn=M[row*2]*(-1*(-(M[p2*2+1]-M[2*row+1])/d2+4*((M[p1*2+1]-M[2*row+1])/d1)+4*(-(M[p_1*2+1]-M[2*row+1])/d_1)+(M[p_2*2+1]-M[2*row+1])/d_2))+M[row*2+1]*(-(M[p2*2+1]-M[2*row])/d2+4*((M[p1*2+1]-M[2*row])/d1)+4*(-(M[p_1*2+1]-M[2*row])/d_1)+(M[p_2*2+1]-M[2*row])/d_2);
		zn=zn/6.0;
	}
...
K[row*2+col]=K[row*2+col]+(zn/r2)*N[row*2+col];
